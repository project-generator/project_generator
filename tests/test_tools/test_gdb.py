...
from project_generator.tools.gdb import ARMNoneEABIGDB

from .simple_project import project_1_yaml, projects_1_yaml

class TestARMNoneEABIGDB(TestCase):
    ...
    def setUp(self):
        ...
        self.armgdb = ARMNoneEABIGDB(self.project.project, ProjectSettings())
    ...
    def test_export_project(self):
        result = self.project.generate('arm_none_eabi_gdb', False)
       # it should get generated files from the last export
        projectfiles = self.project.get_generated_project_files('arm_none_eabi_gdb')

        assert result == 0
        assert projectfiles
        assert os.path.splitext(projectfiles['files'][0])[1] == '.gdbstartup'
