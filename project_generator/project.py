    ...
    def export(self, tool, copy):
        ...
        for export_tool in tools:
            ...
            for project in self.projects:
                ...
                files = tool_export(project.project['common'], self.pgen_workspace.settings).export_project()
...
class Project:
    ...
    def export(self, tool, copy):
        ...
        for export_tool in tools:
            ...
            files = exporter(self.project['common'], self.pgen_workspace.settings).export_project()
    ...
    def copy_sources_to_generated_destination(self):
        self.project['common']['copy_sources'] = True
        self.copy_files()
    ...
    def _set_output_dir(self):
        path = self.project['common']['output_dir']['path']
        self.project['common']['output_dir']['rel_path'], self.project['common']['output_dir']['rel_count'] = self._generate_output_dir(path)
    ...
    def _merge_common_data_with_tool(self, tool):
        ...
        self.project['common']['includes'] = self.project['common']['includes'] + self._get_tool_data('includes', tool_keywords)
        self.project['common']['include_files'] =  self.project['common']['include_files'] + self._get_tool_data('include_files', tool_keywords)
        self.project['common']['source_paths'] =  self.project['common']['source_paths'] + self._get_tool_data('source_paths', tool_keywords)
        self.project['common']['macros'] = self.project['common']['macros'] + self._get_tool_data('macros', tool_keywords)
        self.project['common']['linker_file'] =  self.project['common']['linker_file'] or self._get_tool_data('linker_file', tool_keywords)
        self.project['common']['template'] = self._get_tool_data('template', tool_keywords)
        self.project['common']['misc'] =  self._get_tool_data('misc', tool_keywords)

        # This is magic with sources as they have groups
        tool_sources = self._get_tool_sources(tool_keywords)
        self.project['common']['source_files'] = merge_recursive(self.project['common']['source_groups'], tool_sources)
        for ext in ["c","cpp","s","lib", "obj"]:
           key = "source_files_" + ext
           self.project['common'][key] = merge_recursive(self._source_of_type(self.project['common']['source_groups'], ext), self._source_of_type(tool_sources, ext))

        # linker checkup
        if len(self.project['common']['linker_file']) == 0 and self.project['common']['output_type'] == 'exe':
            raise RuntimeError("Executable - no linker command found.")
        elif self.project['common']['output_type'] == 'exe':
            # There might be a situation when there are more linkers. warn user and choose the first one
            if type(self.project['common']['linker_file']) == type(list()):
                if len(self.project['common']['linker_file']) > 1:
                    logging.debug("More than one linker command file for the project: %s" % self.name)
                self.project['common']['linker_file'] = self.project['common']['linker_file'][0]
    ...
    def _set_output_dir_path(self, tool):
        if self.pgen_workspace.settings.export_location_format != self.pgen_workspace.settings.DEFAULT_EXPORT_LOCATION_FORMAT:
            ...
            if 'export_dir' in self.project['common'] and self.project['common']['export_dir']:
                location_format = self.project['common']['export_dir']
            else:
                location_format = self.pgen_workspace.settings.export_location_format
        ...
        location = PartialFormatter().format(location_format, **{
            'project_name': self.name,
            'tool': tool,
            'target': self.project['common']['target'],
            'workspace': self._get_workspace_name() or '.'
        })

        # TODO (matthewelse): make this return a value directly
        self.project['common']['output_dir']['path'] = os.path.normpath(location)
    ...
    def copy_files(self):
        ...
        for path in self.project['common']['includes']:
            if os.path.isdir(path):
            ...
            dest_dir = os.path.join(os.getcwd(), self.project['common']['output_dir']['path'], path)
            ...
            for filename in files:
                if filename.split('.')[-1] in FILES_EXTENSIONS['includes']:
                    shutil.copy2(os.path.join(os.getcwd(), path, filename),
                                 os.path.join(os.getcwd(), self.project['common']['output_dir']['path'], path))

        # all sources are grouped, therefore treat them as dict
        for k, v in self.project['common']['source_files_c'].items():
            for file in v:
                self._copy_files(file, self.project['common']['output_dir']['path'], FILES_EXTENSIONS['source_files_c'])

        for k, v in self.project['common']['source_files_cpp'].items():
            for file in v:
                self._copy_files(file, self.project['common']['output_dir']['path'], FILES_EXTENSIONS['source_files_cpp'])

        for k, v in self.project['common']['source_files_s'].items():
            for file in v:
                self._copy_files(file, self.project['common']['output_dir']['path'], FILES_EXTENSIONS['source_files_s'])

        for k,v in self.project['common']['source_files_obj'].items():
            for file in v:
                self._copy_files(file, self.project['common']['output_dir']['path'], FILES_EXTENSIONS['source_files_obj'])

        for k,v in self.project['common']['source_files_lib'].items():
            for file in v:
                self._copy_files(file, self.project['common']['output_dir']['path'], FILES_EXTENSIONS['source_files_lib'])

        linker = os.path.normpath(self.project['common']['linker_file'])
        dest_dir = os.path.join(os.getcwd(), self.project['common']['output_dir']['path'], os.path.dirname(linker))
        if not os.path.exists(dest_dir):
            os.makedirs(dest_dir)
        shutil.copy2(os.path.join(os.getcwd(), linker),
                     os.path.join(os.getcwd(), self.project['common']['output_dir']['path'], linker))
